...
- 本輪主線採 Google Maps 即時地點／路線資料與 Sideby 決定性推薦規則；Gemini 不列為本次比賽主打，也不是提交阻斷。需求句用作人工核准後的回歸驗收，候選句不自動升格黃金答案。
...
最新 Lovable 主畫面已接根後端的建立／加入房間、共享條件、私密需求、確認、三套合成行程、反應／重排、定案與本人偏好回饋。既有三套地點／路線已整理成版本化 showcase dataset 與 travel matrix，由 Railway 啟動時冪等寫入 PostgreSQL，生成結果保存 dataset／matrix version；九個站點保存 Google Place ID，商家地址、照片、評分與導航仍由前端即時取得，不寫回資料庫。固定範例不會在生成失敗時冒充新結果；展示價格、時長與屬性明示 synthetic demo。這只證明程式與自動測試整合；公開 Runtime 與雙手機仍需分層驗收。
