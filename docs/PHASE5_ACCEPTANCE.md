...
Phase 5 後端第一刀已實作確定性硬限制、UserFit／CoupleScore、三套差異行程組合、核准資料與作用中交通矩陣讀取，以及 generate／itineraries API。GET 會以嚴格 public schema 重驗資料庫 payload；場地資料集可用同一穩定 venue_id 保存多個版本。測試只使用明確標示的合成場地；fairness 最低門檻、Phase 4 的真實核准場地、RAG 檢索與三案例 Runtime 證據尚未交付，因此 Phase 5 overall 應為 `NOT_READY`。
...
2. 先以確定性程式合併共同與私人日期／時間／戶外／訂位限制，再排除預算、營業、交通、單段／總移動、飲食／過敏、無障礙、年齡／場域、Hard No、戶外／天氣不合格候選；過敏字串需正規化，價格基準等必要事實未知不得放行。
...
5. 公開結果只使用核准 venue record、execution slot 與交通矩陣；GET 必須以嚴格 public schema／allowlist 重建輸出，不包含私密原文、標籤、輸入者、解析結果、任意 JSONB 欄位或可反推理由。
