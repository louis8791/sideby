...
## 2026-09-06 比賽收尾（產品介紹稽核後）

- **必做：**依 `docs/PRODUCT_INTRO_CLAIM_AUDIT.md` 重製最新產品介紹 PDF；舊 PDF 保留為 2026-09-05 本機快照，不得作最新提交版。
- **必做：**兩支實體手機完成建立／加入、共享同步、私密互不可見、雙確認、三套方案、待確認警示、鎖站重排、定案、刷新保存及 Google 成功／失敗降級；至少一支使用行動網路。
- **必做：**公開可見登入入口若保留，須驗證真實成功登入與恢復，否則標示選用／Beta；接著由 Owner 走查正式網址並固定 main commit、部署版本、場地 release、測試、簡報／影片及提交連結。
- **可延後：**1,108 筆場地逐步補證、新模型資料與訓練、自訂 `sideby.dev` 網域、跨城市、交易、負載及商業驗證；這些不得被寫成目前完成能力。

## 2026-09-06 全候選正式發布（已部署）
...
- 已完成正式部署：PR #27 merge `4dbabfa`；Railway active release／index 均為 1,121 筆，13 筆完整驗證、1,108 筆待確認，公開 API 回 3 套三站；Cloudflare version `c0aace76-df4f-4914-b373-db9e956564a3` 已含新 UI 警示。
...
- PR #25 已合併，Cloudflare Worker version `4a13b1cc-6930-4c85-ad64-9059ebc8f914` 已部署；首頁、同源 Runtime、正式 bundle、雙瀏覽器加入及 Google 四項連線／底圖均通過。尚未逐一證明含 Place ID 的候選在當下評論都會命中線索。此功能只增加展示資訊，不提高 13 筆完整驗證場地數，也不構成模型持續學習證據。
...
- 待完成：逐步補齊政府候選營業、價格、區域與主觀觀察證據；人工同意語料及保留題實測；兩支實體手機。1,121 筆現已作為正式比較池，但 1,108 筆須標示待確認，不能冒充完整驗證；學習候選 0，未訓練新模型。
...
- 偏好 UI 目前 34 項；原 30 項已全部映射至 21 個計分屬性，另 4 項是環境硬條件。映射完成不代表 1,121 個場地都有相應屬性證據，未知硬條件仍不得放行。
...
| 舊選項與自由文字 | 30 項已映射至 21 屬性；原文硬限制另保留 | 維持語意與回歸；不得靜默遺失否定、限制或未支援硬需求 |
