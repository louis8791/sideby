...
- 使用者：後端、模型與 RAG 部署、資料、權限、計分、行程驗證與測試。
...
- 本地模型與 RAG 已在目標機執行，候選與來源可查；雲端模型請求為零，失敗狀態真實可見。
...
使用者先做 Phase 1A：確認執行硬體，選定本地生成／Embedding 與索引工具，跑通一批場地的可追溯檢索；同時與前端成員約定公開資料格式。此任務完成前，不把模型型號、資料規模或效能數字寫成已驗證。
