...
export interface PublicItinerary {
  ...
  stops: Array<{
    ...
    travel_minutes: number; estimated_cost: number; locked: false;
...
export const publicItinerarySchema = z.strictObject({
  ...
  stops: z.array(z.strictObject({
    ...
    locked: z.literal(false), booking_status: executionSlotSchema.shape.bookingStatus, booking_url: z.null(),
...
function candidateAllowed(candidate: Candidate, shared: SharedConditions, hard: ReturnType<typeof combinedHard>, queries: Query[]) {
  ...
  if (!shared.outdoorAllowed && slot.outdoor) return false;
  ...
  if (!shared.bookingAllowed && slot.bookingStatus !== 'not_required') return false;
  ...
  if (slot.allergensPresent.some(item => hard.allergens.has(item.toLocaleLowerCase('zh-TW')))) return false;
...
export function composeItineraries(input: {
  ...
  for (const row of input.venues) {
    ...
    if (candidateAllowed(candidate, input.shared, hard, queries)) candidates.push(candidate);
  }
  ...
  const start = Date.parse(input.shared.startsAt), end = Date.parse(input.shared.endsAt);
  ...
  const visit = (chosen: Candidate[], stops: PublicItinerary['stops'], from: string, current: number, cost: number, travel: number) => {
    if (chosen.length === input.shared.stops) {
      ...
      options.push({
        itinerary_id: randomUUID(), session_id: input.sessionId,
        ...
        validation: { hard_constraints_passed: true, privacy_guard_passed: true, source_ids_verified: true,
          checked_at: (input.now ?? new Date()).toISOString(), data_freshness_note: '使用核准資料集與作用中交通矩陣產生。' },
    ...
    for (const candidate of candidates) {
      ...
      visit([...chosen, candidate], [...stops, {
        stop_id: randomUUID(), order_no: chosen.length + 1, venue_id: candidate.venue.venueId,
        ...
        locked: false, booking_status: candidate.slot.bookingStatus, booking_url: null,
