...
- 場地發布、共用排序與 RAG 索引一律先通過 `src/venues/policy.ts`；政府匯入只能建立 draft 骨架。個人回饋不得進共用 RAG，情境屬性不得省略其時段／區域後冒充一般屬性。
...
- 需求訓練資料與場地資料分開；正式使用者私密輸入不得自動回收訓練。訓練使用須另有明確同意與去識別處理；勾選記住偏好不等於同意模型訓練。
- 四份文件已記錄方案；場地資料的 JSON Schema、政府資料正規化匯入器與共用政策守門已有合成測試。這不代表 Python 分類器、模型權重、真實場地或 RAG 索引已交付；第一批房間後端仍依原有 HTTP 證據獨立驗收。
