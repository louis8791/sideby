...
## 2026-09-06 產品介紹宣稱稽核與收尾

- `output/pdf/Sideby_窗口產品介紹.pdf` 是 2026-09-05 的 12 頁本機 MVP 快照，不是目前正式部署證據。逐頁差異及比賽收尾以 `docs/PRODUCT_INTRO_CLAIM_AUDIT.md` 為準。
- 對外現況固定寫法：30 個軟偏好全部映射至 21 個計分屬性，另有 4 個環境硬條件；正式比較池 1,121 筆，其中 13 筆完整驗證、1,108 筆待確認；三條持續改進管線已接通，但有效模型學習候選為 0，沒有新模型或品質提升證據。
- 已公開部署不得再寫成「非已上線服務」，但公開 Runtime、桌面雙瀏覽器與自動測試也不得升格成兩支實體手機或 Owner 接受。比賽提交前必做：更新產品介紹、兩支實體手機完整流程、處理可見登入入口、Owner 最終走查及固定提交版本證據。
- `www.sideby.dev`／`sideby.dev` 於 2026-09-06 尚無可用 DNS／HTTPS；正式入口仍是 Cloudflare Workers 網址。自訂網域是選用品牌工作，不阻擋本次功能提交。
...
- 線索必須標示為「依目前 Google 評論粗略歸類」，只供黑客松畫面比較；不得寫入 PostgreSQL、場地屬性、推薦索引、硬篩選、使用者偏好、RAG、訓練或評測。評論原文與衍生線索均不得跨請求保存。
- 冷氣、價格、室內外、營業與其他硬條件仍只認 Sideby 核准資料；Google 作者、照片與評論署名繼續顯示。PR #25 已合併 `main`（`030de8b`），Cloudflare Worker version `4a13b1cc-6930-4c85-ad64-9059ebc8f914` 已部署；公開首頁與同源 `/api/runtime` 回 200，正式 bundle 含線索標題及免責文字，Google 四項連線與底圖目視通過。實際行程卡出現哪些線索仍取決於當次最多五則評論內容。
...
- 自然語言不得因正規化而丟失否定或硬限制。私人偏好、離線候選、場地索引各自驗收；未有人工核准資料／保留題改善證據不得宣稱模型提升。
- Google 額度須先核對計費與本輪上限；不啟用付費或無限批次。額度未知時先完成零 Google 呼叫的資料處理。
...
- 專用 Free tier 專案的 key 只存於 Cloudflare `GEMINI_API_KEY` secret。2026-09-06 已以兩個獨立瀏覽器驗證完整邀請加入、雙方非敏感測試輸入、Gemini Server Function 200，以及三套 `approved_dataset` 路線產生；這不等於真實私密資料處理或兩支實體手機驗收。
- 免費層 UI 必須明示內容可能依 Google 條款用於改善產品，並要求使用者確認只送非敏感展示內容；不得再顯示「不代表同意訓練」這類與目前供應商層級不一致的承諾。診斷日誌驗收後關閉，不能保存提示內容或持續蒐集不必要的請求中繼資料。
...
- 此段記錄首批 13 筆發布時狀態；目前雙方 UI 仍為 34 個可選項，但原 30 項現已全部映射至 21 個計分屬性，另 4 項是環境硬限制。偏好可辨識與場地是否有相應屬性證據仍不可混稱。
...
- 政府匯入仍只建立客觀 draft，不推論主觀偏好、價格、營業、室內外或冷氣；首批只有 Owner 明確列入的 13 筆進完整驗證版本。其後 1,121 筆已依最新全候選政策進正式比較池，但 1,108 筆必須維持待確認，不能冒充完整核准。
...
- 雙方使用相同的 34 個選項：既有氛圍 12、狀態 8、互動 10，新增環境 4；完整清單與目前映射見 PRD。此段原為 2026-09-05 歷史狀態；目前 30 個軟選項已全部映射至 21 個計分屬性，仍不得把「可辨識」寫成「每個場地都有證據可滿足」。
